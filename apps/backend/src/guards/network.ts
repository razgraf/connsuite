...
import { limits, networks, WITH_POLICY } from "./atoms";
...
export const policy = {
  ...
  icon: {
    ...
    2: `Please use files smaller than ${limits.MAX_NETWORK_ICON_SIZE / (1024 * 1024)} MB.`,
...
function isNetworkTitleAcceptable(value: string, withPolicy = WITH_POLICY): string | boolean {
  const [min, max] = [limits.MIN_NETWORK_TITLE_LENGTH, limits.MAX_NETWORK_TITLE_LENGTH];
...
function isNetworkUsernameAcceptable(value: string, withPolicy = WITH_POLICY): string | boolean {
  const [min, max] = [limits.MIN_NETWORK_USERNAME_LENGTH, limits.MAX_NETWORK_USERNAME_LENGTH];
  if (_.isNil(value) || _.isEmpty(value) || (!_.isString(value) && !_.isNumber(value)))
...
function isNetworkExternalIdAcceptable(value: string, withPolicy = WITH_POLICY): string | boolean {
  if (_.isNil(value) || _.isEmpty(value) || !_.isString(value)) return withPolicy ? policy.externalId[1] : false;
  if (!Object.keys(networks).includes(value)) return withPolicy ? policy.externalId[2] : false;
...
function isNetworkIconAcceptable(
  ...
  if (options && options.vendor === "multer") {
    ...
    if (_.isNil(name) || _.isEmpty(name) || !_.isString(name)) return withPolicy ? policy.icon[3] : false;
    if (_.isError(type) || !limits.ALLOWED_NETWORK_ICON_FORMAT.includes(type || ""))
      return withPolicy ? policy.icon[1] : false;
    if (_.isError(size) || size <= 0 || size > limits.MAX_NETWORK_ICON_SIZE) return withPolicy ? policy.icon[2] : false;
    return true;
  } else {
    /** Client Side */
...
function isNetworkDescriptionsAcceptable(value: string, withPolicy = WITH_POLICY): string | boolean {
  const [min, max] = [limits.MIN_NETWORK_DESCRIPTION_LENGTH, limits.MAX_NETWORK_DESCRIPTION_LENGTH];
  if (_.isNil(value) || _.isEmpty(value) || !_.isString(value)) return withPolicy ? policy.description.root : false;
  return value.length >= min && value.length <= max ? true : withPolicy ? policy.description[1] : false;
}
