...
function SpinnerCircle({ className, color, isVisible, size, thickness }) {
  return <SpinnerCircleWrapper data-visible={isVisible} className={className} color={color} size={size} thickness={thickness} />;
}

function Spinner({ className, color: rawColor, type = "circle", ...otherProps }) {
  const color = useMemo(() => (_.isFunction(rawColor) ? rawColor(colors) : _.toString(rawColor)), [rawColor]);

  switch (type) {
    case "circle":
      return <SpinnerCircle color={color} className={className} {...otherProps} />;
    default:
      return <></>;
  }
}
