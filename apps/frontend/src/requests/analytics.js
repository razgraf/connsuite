...
async function visitGet({ auth, type = types.visit.type.network, targetId, from = null, to = null }, handle = true) {
  const endpoint = new URL(API.analyticsVisitGet(type, targetId));
  const params = {
    from,
    to,
  };
...
async function visitList({ auth, type = types.visit.type.network, from = null, to = null }, handle = true) {
  const endpoint = new URL(API.analyticsVisitList(type));
  const params = {
    from,
    to,
  };
