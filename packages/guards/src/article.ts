...
export const policy = {
  ...
  cover: {
    root: `Covers must be images (${limits.ALLOWED_ARTICLE_COVER_FORMAT.join(
      ", "
  ...
  categories: {
    ...
    1: `You should have between ${limits.MIN_ARTICLE_CATEGORIES_COUNT} and ${limits.MAX_ARTICLE_CATEGORIES_COUNT} categories per article.`,
    2: `Custom categories should have between ${limits.MIN_ARTICLE_CATEGORIES_TITLE_LENGTH} and ${limits.MAX_ARTICLE_CATEGORIES_TITLE_LENGTH} characters`,
  },
  content: {
...
function isArticleTitleAcceptable(
  value: string,
  withPolicy = WITH_POLICY
): string | boolean {
  const [min, max] = [
    limits.MIN_ARTICLE_TITLE_LENGTH,
    limits.MAX_ARTICLE_TITLE_LENGTH,
  ];
  if (
    _.isNil(value) ||
    _.isEmpty(value) ||
...
function isArticleCoverAcceptable(
  ...
  if (options && options.vendor === "multer") {
    ...
    if (_.isNil(name) || _.isEmpty(name) || !_.isString(name))
      return withPolicy ? policy.cover[3] : false;
    if (
      _.isError(type) ||
      !limits.ALLOWED_ARTICLE_COVER_FORMAT.includes(type || "")
    )
...
function isArticleUrlAcceptable(
  value: string,
  withPolicy = WITH_POLICY
): string | boolean {
...
function isArticleContentAcceptable(
  value: string,
  withPolicy = WITH_POLICY,
  isStringified = false
): string | boolean {
...
function isArticleSkillListAcceptable(
  ...
  if (_.isNil(list) || !_.isArray(list))
    return withPolicy ? policy.skills.root : false;
  if (list.length > limits.MAX_ARTICLE_SKILLS_COUNT)
    return withPolicy ? policy.skills[1] : false;

  const [min, max] = [
    limits.MIN_ARTICLE_SKILLS_TITLE_LENGTH,
    limits.MAX_ARTICLE_SKILLS_TITLE_LENGTH,
  ];

  for (let i = 0; i < list.length; i += 1) {
    const skill = list[i];
    if (!_.has(skill, "_id") && !_.has(skill, "title"))
      return withPolicy ? policy.skills[2] : false;
    if (_.has(skill, "_id") && !_.get(skill, "_id"))
      return withPolicy ? policy.skills[2] : false;
...
function isArticleCategoryListAcceptable(
  value: any[] | string | undefined,
  withPolicy = WITH_POLICY,
  isStringified = false
): string | boolean {
  ...
  if (_.isNil(list) || !_.isArray(list))
    return withPolicy ? policy.categories.root : false;
  if (
    limits.MIN_ARTICLE_CATEGORIES_COUNT > list.length ||
    list.length > limits.MAX_ARTICLE_CATEGORIES_COUNT
  )
    return withPolicy ? policy.categories[1] : false;

  const [min, max] = [
    limits.MIN_ARTICLE_CATEGORIES_TITLE_LENGTH,
    limits.MAX_ARTICLE_CATEGORIES_TITLE_LENGTH,
  ];

  for (let i = 0; i < list.length; i += 1) {
    const category = list[i];
    if (!_.has(category, "_id") && !_.has(category, "title"))
      return withPolicy ? policy.categories[2] : false;
    if (_.has(category, "_id") && !_.get(category, "_id"))
      return withPolicy ? policy.categories[2] : false;
    if (_.has(category, "title")) {
      const length = _.toString(_.get(category, "title")).length;
      if (length < min || length > max)
        return withPolicy ? policy.categories[2] : false;
    }
  }
