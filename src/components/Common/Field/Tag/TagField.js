...
class TagField extends BaseField{
    ...
    field = () => {
        return <div className={Helper.dynamicClass(stylesDefault, styles,"field", "TagField")}>
            <div className={Helper.dynamicClass(stylesDefault, styles,"content")}>
                <div className={Helper.dynamicClass(styles, this.props.style,"box")}
                     onClick={(e) => { this.element.field.current.focus();  }}
                >
                    {
                            ...
                            this.props.data.value.map((element, index) => {
                                return (
                                    <li
                                        ...
                                        <Icon icon round source={"close"} className={styles.icon} onClick={()=>{
                    ...
                    <li className={styles.input}>
                        <input
                            ...
                            onFocus={(e)=>{ if(!Helper.isEmpty(e.target.value) && !this.state.tagField_isDatasetVisible) this.setState({tagField_isDatasetVisible : true}) }}
                            onChange={(e) => {
                                this.setState({tagField_input : e.target.value});
                                if(!Helper.isEmpty(e.target.value) && !this.state.tagField_isDatasetVisible) this.setState({tagField_isDatasetVisible : true})
                            }}
                            onDoubleClick={()=>{
                                if(!this.state.tagField_isDatasetVisible) this.setState({
                            ...
                            onKeyDown={(e)=>{
                                if(((typeof e.which == "number") ? e.which : e.keyCode) === 32){
                                     ...
                                     * Backspace
                                     */
                                    if(Helper.isEmpty(this.state.tagField_input) && !Helper.isEmpty(this.props.data.value) && this.props.data.value.length > 0){
                    ...
                    />
                        ...
                        <FieldDataset
                            ...
                            source={this.props.data.source.filter(element => { for(let i = 0; i < this.props.data.value.length; i++) if(this.props.data.value[i].title.toLowerCase() === element.title.toLowerCase()) return false; return true; } )}
