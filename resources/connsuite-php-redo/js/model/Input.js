...
class Input extends ClassHelper {

    /**
     * used when sending key-value pairs, to be different than ID if needed
     * @returns {String}
     */
    get key() {
        return this._key;
    ...
    constructor(object){
        ...
        this.isGroup = false;
        this.custom = this.getValue(object, INPUT_KEY_CUSTOM) ? this.getValue(object, INPUT_KEY_CUSTOM) : false;
        this.flag = INPUT_FLAG_NORMAL;
        this.initialized = this.getValue(object,INPUT_KEY_INITIALIZED) ? this.getValue(object,INPUT_KEY_INITIALIZED) : false;
        this.parent = this.getValue(object,INPUT_KEY_PARENT);
        this.type = this.getValue(object, INPUT_KEY_TYPE);

        this.key = this.getValue(object, INPUT_KEY_KEY);

        if(this.initialized){
            this.element = this.getValue(object, INPUT_KEY_ELEMENT);
            this.label = this.element.siblings("p").length > 0 ? this.element.siblings("p").text() : "";
            ...
            this.ID = this.getValue(object, INPUT_KEY_ID);
            this.class = this.getValue(object, INPUT_KEY_CLASS);
            this.name = this.getValue(object, INPUT_KEY_NAME);
            this.extraData = this.getValue(object, INPUT_KEY_EXTRA_DATA);
            this.placeholder = this.getValue(object, INPUT_KEY_PLACEHOLDER);
            this.maxlength = this.getValue(object, INPUT_KEY_MAXLENGTH);
            this.value = this.getValue(object, INPUT_KEY_VALUE);
            this.label = this.getValue(object, INPUT_KEY_LABEL);
        }


        this.danger = this.getValue(object,INPUT_KEY_DANGER);
        this.callback = object.hasOwnProperty("callback") ? object["callback"] : null;
        this.source = this.getArray(object, INPUT_KEY_SOURCE);
...
class TextInput extends Input{
    ...
    constructor(object) {
        ...
        this.area = !!this.getValue(object, "area");
    ...
    standardPrint() {
        ...
        let code =  '<div class="input"> ' +
            (this.area ?
            '<textarea  type="text"  '+O.ID+' '+O.class+' '+O.name+' '+O.extraData+' '+O.maxlength+'  placeholder="'+O.placeholder+'"></textarea> ':
            '<input  type="text"  '+O.ID+' '+O.class+' '+O.name+' '+O.extraData+' '+O.maxlength+' value=""   placeholder="'+O.placeholder+'"/> '
...
class NumberInput extends Input{
    ...
    constructor(object) {
        ...
        this.isPhone = this.getValue(object,"isPhone");
        if (this.parent && !this.initialized) this.standardPrint();
        if(this.callback) this.callback(this);
    }
...
class CheckboxInput extends Input{
    ...
    isChecked(){
        return this.element && this.element.length > 0 ? this.element.is(":checked")  : false;
    }
...
class RadioGroup extends ClassHelper{
    ...
    constructor(nameID, parent, label, initial,boxes = [], element = null, danger = INPUT_DANGER_BIG){
        super();
        this.ID = nameID;
        this.isGroup = true;
        this.parent = parent;
...
class ImageInput extends Input{
    ...
    constructor(object) {
        ...
        this.defaultImage = this.getValue(object,"defaultImage"); if(isEmpty(this.defaultImage)) this.defaultImage = PATH_DEFAULT_PICTURE_PERSON;
    ...
    standardPrint() {
        ...
        let code =
            ...
            '<div class="upload-button"><span>Choose</span></div> ' +
            '<div class="upload-button-remove"><span>Remove</span></div> ' +
            '</div> ' +
            '<p>'+O.label+'</p> ' +
            '</div>';
        ...
        if(this.parent.find($("#"+this.ID))) {
            ...
            if(this.value){
               if(this.preview.length > 0) {
                   bindProfilePicture(this.preview, this.value);
               }
               this.element.val("");
            }
        }
    ...
    init(){
        ...
        this.element.unbind().on("change",function(event){
            if( event.target.files.length > 0 &&
                ...
                if(event.target.files[0].size > 3000000) showAlert("Please upload a file < 5MB",ALERT_TYPE_FAILURE,1000);
                self.preview.prop("src",self.defaultImage);
                disableButtonClick(removeButton);
            }
        });
