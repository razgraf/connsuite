...
function lazyLoadNewsFeed(nearToBottom, element, parent){
    ...
    LOGGER.log("DocHeight : "+ $(document).height());
    LOGGER.log("E Scroll Top : "+ element.scrollTop());
    LOGGER.log("E Height : "+ element.height() );
    LOGGER.log("W InnerHeight : "+ parent.height());
...
function disableButtonClick(element, loader = false){
    if(!element.length){
        LOGGER.log("Element missing on disable.");
        LOGGER.log(element);
        return;
    }
...
function bindProfilePicture(parent, url, defaultURL = PATH_DEFAULT_PICTURE_PERSON ){
    ...
    imageExists(url,function(callback){
        if(!callback) url = defaultURL;
        parent.attr("src", url).fadeIn(100);
    });
}
...
function isDateBefore(shouldBeSmaller, shouldBeBigger){
    if(isEmpty(shouldBeSmaller) || isEmpty(shouldBeBigger)) return false;
    else{
        try{
            moment.locale('en-gb');
            if( parseInt(moment(shouldBeSmaller,"YYYY-MM-DD").diff(moment(shouldBeBigger, "YYYY-MM-DD"),'minutes')) <= 0){
                return true;
            }
        }
...
class LOGGER{
    ...
    static log(value){try {if(!RELEASE) console.log(value);}catch (e){}}
    static table(value){   try {if(!RELEASE) console.table(value);}catch (e){}}
