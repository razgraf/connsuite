...
$conn = $E -> validateAPIConnection(URL_NETWORK_ADD, true, true);

include_once($E->goIdentifierToIdentifier(URL_NETWORK_ADD, SYSTEM_IDENTIFIER_LIBRARY_IMAGE));
...
$n[NETWORK_KEY_LINK_AID] = Environment::createUniqueAID($conn,"network_link","AID");



switch($n[NETWORK_KEY_TYPE])    {
    case NETWORK_TYPE_DEFAULT :
        $n[NETWORK_KEY_ID] =  (int) getAPIParam($_POST[NETWORK_KEY_ID], DATA_TYPE_NUMERIC,"Missing ID.");
        $n[NETWORK_KEY_USERNAME] =  getAPIParam($_POST[NETWORK_KEY_USERNAME], DATA_TYPE_STRING,"Missing username.");

        /**
         * Check if ID exists in network_default table | FAIL-SAFE
         */
        $checker = PDOParser::parseBooleanValidRows($conn,"SELECT ID FROM network_default WHERE ID= :ID",array([":ID",$n[NETWORK_KEY_ID],PDO::PARAM_INT]));
        if(!$checker) exit(exitWithNegativeDebug("Missing default network."));
