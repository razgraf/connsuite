...
class NavUserProfile extends Component {
  ...
  render() {
    ...
    let routeBack =
      !Helper.isEmpty(page) && !Helper.isEmpty(page.routeBack) ? page.routeBack : Config.ROUTE_PAGE_DASHBOARD;

    let onBackClick = () => {
      this.props.history.push(routeBack);
    };
    ...
    return (
      <nav
        className={cx(styles.NavUser, styles.NavUserProfile)}
        data-top={this.state.scrollY <= NavUserProfile.Y_TRIGGER}
      >
        <div className={styles.container}>
          <div
            ...
            data-back={withReturn}
          >
            <Icon image className={styles.default} source={require("../../../../assets/images/logo.png")} alt={""} />
            {withReturn ? <Icon icon className={styles.back} source={"arrow_back"} alt={"Back"} /> : null}
          </div>

          <div className={styles.content}>
            <div className={styles.steps}>
              <div className={styles.title}>
                <p>{this.props.active.name + "'s"}</p>
              </div>
              <div
                ...
                onClick={() => {
                  this.setState({ isFlowBoxVisible: !this.state.isFlowBoxVisible });
                }}
              >
                <p>{this.state.steps[this.props.activeStep].title}</p>
                <Icon icon className={styles.icon} source={"keyboard_arrow_down"} />
                ...
                <div className={styles.flowBox} ref={this.flowBox} data-visible={this.state.isFlowBoxVisible}>
                  {this.state.steps.map((element, index) => (
                    <HashLink
                      ...
                      key={index}
                      onClick={() => {
                        this.setState({ isFlowBoxVisible: false });
                      }}
              ...
              <div className={styles.list}>
                {this.props.active.networks.slice(0, 4).map((element, index) => (
                  <NetworkMini
                    ...
                    network={element}
                    onClick={() => {
                      this.props.onViewNetworkClick(element);
                    }}
                  />
                ))}
...
export default compose(
  ...
  connect(
    ...
    dispatch => {
      return {
        onViewNetworkClick: network => {
          return dispatch({ type: Config.REDUX_ACTION_CONTROLLER_COVER_NETWORK_CHOOSE, payload: { network: network } });
        },
        onClose: () => {
          return dispatch({ type: Config.REDUX_ACTION_CONTROLLER_COVER_CLOSE, payload: { visible: false } });
        },
        ...
        updateHistory: history => {
          return dispatch({ type: Config.REDUX_ACTION_VIEW_NAVIGATOR_SET_HISTORY, payload: { history: history } });
        },
      };
    },
  ),
