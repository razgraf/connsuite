...
class FileField extends BaseField {
  ...
  field = () => {
    return (
      <div className={Helper.dynamicClass(stylesDefault, stylesSpecific, "field", "FileField")}>
        <div
        ...
        >
          <input
            type={"file"}
            accept={
              this.props.data.fileType === "image"
                ? "image/*"
                : this.props.data.fileType === "file"
                ? ".pdf, .doc, .docx"
                : null
            }
          ...
          <Button
            custom={{ style: stylesSpecific, className: "buttonUpload" }}
  ...
  isValid = handleWarn => {
    ...
    if (!FileField.isValidSize(value)) {
      if (!flag)
        this.setState({
          warnText: `Select a file smaller than ${Math.round(this.props.data.maxFileSize / (1024 * 1024))} MB `,
        });
      flag = true;
    }
