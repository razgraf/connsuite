...
class NetworkManagerEdit extends BasePage {
  ...
  retrieve = () => {
    return new Promise(resolve => {
      ...
      for (let i = 0; i < networks.length; i++)
        if (networks[i].AID === this.state.AID) resolve(NetworkModel.clone(networks[i]));
      resolve(null);
    });
  };
        ...
        <div className={styles.container}>
          <div className={styles.content}>
            <div className={styles.header}>
              <div className={styles.steps}>{this.state.steps.map(step => this.buildStep(step))}</div>
            </div>
                    ...
                    {this.state.activeSection === 0 ? (
                      <section className={styles.default}>
                        ...
                        <div className={styles.subtitle}>
                          <p>
                            If you want to edit the info regarding your network here's the place to do it. Make sure
                            that the modifications you have in mind are necessary as changes will directly affect your
                            profile.
                          </p>
                        </div>
                        <div className={styles.content}>
                          <Form
                            ...
                            onUpdate={formState => {
                              ...
                              n.username =
                                !Helper.isEmpty(username) && !Helper.isEmpty(username.data.value)
                                  ? username.data.value
                                  : null;

                              let description = formState.findFieldByID(NetworkManagerEdit.FIELD_COMMON_DESCRIPTION);
                              n.description =
                                !Helper.isEmpty(description) && !Helper.isEmpty(description.data.value)
                        ...
                        <div className={styles.content}>
                          <Form
                            ...
                            fields={[
                              {
                                ...
                                label: {
                                  value: "Title",
                                  help:
                                    "Just like 'Facebook' or 'Twitter' you can give a name to your website/new network.",
                                  force: "right",
                                },
                                warn: {
                                  onBlur: true,
                              ...
                              },
                              {
                                ...
                                label: {
                                  value: "Full link/URL",
                                  help:
                                    "We need a link for the website so we know where to send people that click on the card.",
                                  force: "right",
                                },
                                warn: {
                                  onBlur: true,
                              ...
                              },
                              {
                                ...
                                label: {
                                  ...
                                  help:
                                    "You know how every social network asks for a username? What would yours be for this new custom network?",
                                  force: "right",
                                },
                                warn: {
                                  onBlur: false,
                            ...
                            onUpdate={formState => {
                              ...
                              n.icon.file =
                                !Helper.isEmpty(icon) && !Helper.isEmpty(icon.data.value) ? icon.data.value : null;

                              let title = formState.findFieldByID(NetworkManagerEdit.FIELD_CUSTOM_TITLE);
                              n.title =
                                !Helper.isEmpty(title) && !Helper.isEmpty(title.data.value) ? title.data.value : null;
                  ...
                  <div className={styles.label} data-highlight={true}>
                    <p>
                      {!Helper.isEmpty(active) && !Helper.isEmpty(active.URL)
                        ? active.URL +
                          (this.state.activeSection === 0 && !Helper.isEmpty(active.username) ? active.username : "")
                        : "http://preview"}
                    </p>
                  </div>
                </div>
                  ...
                  <div className={styles.content}>
                    ...
                    <div className={styles.group}>
                      <div className={styles.item}>
                        <NetworkMini css={{ width: "60px", height: "60px" }} viewonly network={active} />
                      </div>
                      <div className={styles.item}>
                        <NetworkMini css={{ width: "60px", height: "60px" }} viewonly network={active} />
                      </div>
                      <div className={styles.item}>
                        <NetworkMini css={{ width: "50px", height: "50px" }} viewonly network={active} />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className={styles.footer}>
              <Button
                custom={{ style: styles, className: "buttonCancel" }}
                title={"Cancel"}
              ...
              <Button
                custom={{ style: styles, className: "buttonRight" }}
  ...
  buildStep(step) {
    return (
      <div
        ...
        data-active={this.state.activeStep === step.id}
  ...
  goOneStepForward = () => {
    if (this.validateStep()) this.goLive();
  };
  ...
  validateStep = (handleWarn = true) => {
    ...
    if (Helper.isEmpty(this.state.network)) {
      flag = false;
    }
